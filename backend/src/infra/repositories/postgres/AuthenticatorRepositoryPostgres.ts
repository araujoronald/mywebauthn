...
export default class AuthenticatorRepositoryPostgres implements AuthenticatorRepository {
    ...
    async find(id: string): Promise<Authenticator> {
        const found = await dbPostgres.oneOrNone('SELECT * FROM authenticators a INNER JOIN users u ON u.id = a.user_id WHERE a.id=$1', [id])
    ...
    async save(authenticator: Authenticator): Promise<Authenticator> {
        const found = await this.find(authenticator.id)
        if (found != null) {
            await dbPostgres.none('UPDATE authenticators SET "counter"=$1 WHERE id=$2', [authenticator.counter, authenticator.id])
        } else {
